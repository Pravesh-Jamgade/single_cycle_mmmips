 ...
 *                      Complete mmMIPS implementation.
 *                      (instructions: lw, sw, beq, add,
 *                      sub, and, or, slt)
 */
...
int sc_main(int argc, char *argv[])
{
    ...
    sc_signal< sc_bv<DWORD>         > bus_imem;

    sc_signal< sc_bv<DWORD>         > bus_dmem;
     ...
     *  Initialization
     */ 
    imem.init("mips_rom.bin");
    dmem.init("mips_ram.bin");
    ...
    sc_trace(tf, bus_imem,                  "bus_imem");
    sc_trace(tf, bus_dmem,                  "bus_dmem");
     ...
     *  Simulation
     */
    unsigned int sim_time = 2000;
    if (argc == 2) sim_time = atoi(argv[1]);

    sc_start(sim_time);
    
    sc_close_vcd_trace_file(tf);
    dmem.dump("mips_ram.dump");
